...
//                }
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        File installSource = null;
        File apkFile = null;
        List<File> splitApks = Collections.emptyList();
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                installSource = new File(
                        BEnvironment.getCacheDir(),
                        UUID.randomUUID().toString() + ".apk"
                );
                InputStream inputStream = BlackBoxCore.getContext()
                        .getContentResolver()
                        .openInputStream(Uri.parse(file));
                FileUtils.copyFile(inputStream, installSource);
            } else {
                installSource = new File(file);
            }

            if (installSource.isDirectory()) {
                apkFile = findClusterBaseApk(installSource);
                if (apkFile == null) {
                    return result.installError("APK cluster has no base.apk");
                }
                splitApks = findClusterSplitApks(installSource, apkFile);
                Slog.i(
                        TAG,
                        "Installing private APK cluster base=" + apkFile.getAbsolutePath()
                                + " splitCount=" + splitApks.size()
                );
            } else {
                apkFile = installSource;
            }

            if (option.isFlag(InstallOption.FLAG_XPOSED) && userId != BUserHandle.USER_XPOSED) {
                return new InstallResult().installError(
                        "Please install the XP module in XP module management"
                );
            }
            ...
            PackageInfo packageArchiveInfo = BlackBoxCore.getPackageManager()
            ...
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo
                        .loadLabel(BlackBoxCore.getPackageManager())
                        + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(
                        packageArchiveInfo.packageName,
                        msg + (BlackBoxCore.is64Bit()
                                ? " not support armeabi-v7a abi"
                                : "not support arm64-v8a abi")
                );
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = BlackBoxCore.getPackageManager()
                        .getPackageInfo(aPackage.packageName, 0)
                        .applicationInfo;
            }
            ...
            int installCode = BPackageInstallerService.get()
                    .installPackageAsUser(bPackageSettings, userId);
            if (installCode < 0) {
            ...
            t.printStackTrace();
            return result.installError("install exception: " + t.getClass().getSimpleName());
        } finally {
            if (installSource != null && option.isFlag(InstallOption.FLAG_URI_FILE)) {
                FileUtils.deleteDir(installSource);
            }
            Slog.d(TAG, "install finish: " + (System.currentTimeMillis() - startedAt) + "ms");
        }
    }

    private File findClusterBaseApk(File directory) {
        File exact = new File(directory, "base.apk");
        if (exact.isFile()) return exact;

        File[] apks = directory.listFiles();
        if (apks == null) return null;
        for (File candidate : apks) {
            if (candidate.isFile()
                    && candidate.getName().toLowerCase().endsWith(".apk")
                    && candidate.getName().toLowerCase().startsWith("base")) {
                return candidate;
            }
        }
        return null;
    ...
    private List<File> findClusterSplitApks(File directory, File baseApk) {
        ArrayList<File> splits = new ArrayList<>();
        File[] files = directory.listFiles();
        if (files == null) return splits;

        for (File candidate : files) {
            if (!candidate.isFile()) continue;
            if (candidate.equals(baseApk)) continue;
            if (!candidate.getName().toLowerCase().endsWith(".apk")) continue;
